...
class Poll
	...
	def userstring(participant, link)
		...
		if link
			...
			ret += '<a title="'
			ret += format(_('Edit user %<user>s...'), user: CGI.escapeHTML(participant))
			ret += '" aria-label="'
			ret += format(_('Edit user %<user>s...'), user: CGI.escapeHTML(participant))
			ret += "\" href=\"?edituser=#{CGI.escape(participant)}\">"
			ret += EDIT
			ret += '</a> | <a title="'
			ret += format(_('Delete user %<user>s...'), user: CGI.escapeHTML(participant))
			ret += '" aria-label="'
			ret += format(_('Delete user %<user>s...'), user: CGI.escapeHTML(participant))
			...
			if $cgi['edituser'] == participant
				...
				@head.columns.each { |column|
					...
					when /yes/ # allow anything containing yes (backward compatibility)
						value = YES
						klasse = YESVAL
						str = format(_('%<user>s selected Yes'), user: CGI.escapeHTML(participant))
					when /no/
						value = NO
						klasse = NOVAL
						str = format(_('%<user>s selected No'), user: CGI.escapeHTML(participant))
					when /maybe/
						value = MAYBE
						klasse = MAYBEVAL
						str = format(_('%<user>s selected Maybe'), user: CGI.escapeHTML(participant))
					end
...
END
	...
	def deleteuser_to_html
		...
		ret += format(_('Do you really want to delete user %<user>s?'), user: CGI.escapeHTML($cgi['edituser']))
	...
	def comment_to_html(editable = true)
		...
		unless @comment.empty?
			...
			c.each { |time, name, comment|
				...
				i += 1
				ret += format(_('%<user>s said on %<time>s'), user: name, time: time.strftime('%d.%m., %H:%M'))
				if editable
...
FORM
	  ...
	  ['ac', _('Access control related')]].each { |value, opt|
...
FORM
	...
	def delete_column(column)
		return false unless @head.delete_column(column)
